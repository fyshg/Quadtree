...
class RegionQuadtree {
    ...
    public: 
        ...
        RegionQuadtree(const RegionQuadtree& other) : root_node(
                std::make_shared<Node>(*(other.root_node))), dim(other.dim) {
        }
        ...
        RegionQuadtree(const Iterator& it_begin, const Iterator& it_end) :   
                root_node(std::make_shared<Node>(it_begin, it_end, 
                        it_end - it_begin, 0, 0)), dim(it_end - it_begin) {  

                        //check if its quadratic with sidelength 2^k
                        assert(dim > 0 && ceil(log2(dim) == floor(log2(dim))));
                        for(std::size_t i = 0; i < dim; i++){
                                auto end = std::end(it_begin[i]);
                                auto begin = std::begin(it_begin[i]);
                                auto N = end - begin ; 
                                assert(N > 0 && 
        ...
        template<typename T>
        RegionQuadtree(T (&arr)[1][1]) : root_node(std::make_shared<Node>(
                arr[1][1] ? BLACK : WHITE)) {
        }

        // Methods for static Array
        template<std::size_t N, typename T>
        RegionQuadtree(T (&arr)[N][N]) : RegionQuadtree(std::begin(arr),
                std::end(arr)){
        }
    ...
    private:
        struct Node{
            public:
                ...
                Node(Node& other) : colour(other.colour) {
                        if(other.colour == GREY){
                              north_east = std::make_shared<Node>(
                                        *(other.north_east));
                              south_east = std::make_shared<Node>(
                                        *(other.south_east));
                              south_west = std::make_shared<Node>(
                                        *(other.south_west));
                              north_west = std::make_shared<Node>(
                                        *(other.north_west));
                       }   
                }


                template<typename Iterator>  //RAC iterators
                Node(const Iterator& it_begin, const Iterator& it_end, 
                        ...
                        if (N == 1 ){
                                ...
                                TreePtr n_e = std::make_shared<Node>(it_begin, 
                                        it_end, N, N + x , y );   
                                TreePtr s_e = std::make_shared<Node>(it_begin,
                                         it_end, N, N + x, N + y ); 
                                TreePtr s_w = std::make_shared<Node>(it_begin, 
                                        it_end, N, x , N + y );
                                TreePtr n_w = std::make_shared<Node>(it_begin, 
                                        it_end, N, x, y );
                ...
                void get_array(std::size_t N, std::size_t x, std::size_t y, 
                            std::vector<std::vector<bool>>& arr){
                        
                        if( colour == GREY) {
                                        N = N/2;
                                        north_east->get_array(N, N+x, y, arr);
                                        south_east->get_array(N, N + x, N + y, 
                                                arr);
                ...
                std::size_t number_of_leaves(){
                        if(colour == BLACK || colour == WHITE){
                                ...
                                return  north_east->number_of_leaves() +
                                        south_east->number_of_leaves() + 
                                        south_west->number_of_leaves() + 
                                        north_west->number_of_leaves();
                        }
                        return 0; 
                }
            private:
